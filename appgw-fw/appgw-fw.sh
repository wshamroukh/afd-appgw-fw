...
az network firewall policy rule-collection-group collection rule add -g $rg -n appsvc-pe-to-appgw-traffic --policy-name $hub_vnet_name-fw-policy --rule-collection-group-name $hub_vnet_name-RuleCollectionGroup  --collection-name $hub_vnet_name-NetworkRuleCollection --rule-type NetworkRule --dest-addr $spoke1_appgw_subnet_address --destination-ports '*' --ip-protocols any --source-addresses $spoke1_pe_subnet_address -o none
...
echo -e "\e[1;36mCreating $spoke1_appgw_name UDR....\e[0m"
az network route-table create -g $rg -n $spoke1_pe_subnet_name -l $location --disable-bgp-route-propagation false -o none
az network route-table route create -g $rg -n default --address-prefix "0.0.0.0/0" --next-hop-type VirtualAppliance --route-table-name $spoke1_pe_subnet_name --next-hop-ip-address $hub1_fw_private_ip -o none
az network route-table route create -g $rg -n to-$spoke1_appgw_name --address-prefix $spoke1_appgw_subnet_address --next-hop-type VirtualAppliance --route-table-name $spoke1_pe_subnet_name --next-hop-ip-address $hub1_fw_private_ip -o none
az network vnet subnet update -g $rg -n $spoke1_pe_subnet_name --vnet-name $spoke1_vnet_name --route-table $spoke1_pe_subnet_name -o none
